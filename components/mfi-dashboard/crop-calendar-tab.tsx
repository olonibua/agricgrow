    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Optimal planting and harvesting times for different crops across Nigeria</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Select Climate Zone</label>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant={isSuitable() ? "default" : "destructive"} className="px-3 py-1">
                {isSuitable() ? "Suitable" : "Not Ideal"}
              </Badge>
              <p className="text-sm">
                {isSuitable() 
                  ? `${selectedCrop.charAt(0).toUpperCase() + selectedCrop.slice(1)} is suitable for cultivation in the ${zoneInfo?.name}.` 
                  : `${selectedCrop.charAt(0).toUpperCase() + selectedCrop.slice(1)} may not be ideal for the ${zoneInfo?.name} without additional measures.`}
              </p>
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{cropInfo?.durationDays} days</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{cropInfo?.needsIrrigation ? "Yes" : "No"}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-sm font-medium mb-2">Climate Zone Information</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  <CloudRain className="h-4 w-4 text-muted-foreground" />
                  <div>
                    <p className="text-sm font-medium">Annual Rainfall</p>
                    <p className="text-sm text-muted-foreground">{zoneInfo?.annualRainfall}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="mt-8">
              ...
              {monthsData.map((month) => (
                <div 
                  ...
                  className={`p-2 rounded-md border text-center ${
                    month.isPlanting && month.isRainy ? 'bg-green-100 border-green-300 dark:bg-green-900/30 dark:border-green-800' : 
                    month.isPlanting ? 'bg-blue-100 border-blue-300 dark:bg-blue-900/30 dark:border-blue-800' :
                    month.isHarvesting ? 'bg-amber-100 border-amber-300 dark:bg-amber-900/30 dark:border-amber-800' :
                    month.isRainy ? 'bg-sky-100 border-sky-300 dark:bg-sky-900/30 dark:border-sky-800' :
                ...
                >
                  ...
                  <div className="flex flex-wrap justify-center gap-1 mt-1">
                    {month.isPlanting && (
                      <Badge variant="outline" className="text-xs px-1 py-0 h-4 bg-blue-100 dark:bg-blue-900/30">Plant</Badge>
                    )}
                    {month.isHarvesting && (
                      <Badge variant="outline" className="text-xs px-1 py-0 h-4 bg-amber-100 dark:bg-amber-900/30">Harvest</Badge>
                    )}
                    {month.isRainy && (
                      <Badge variant="outline" className="text-xs px-1 py-0 h-4 bg-sky-100 dark:bg-sky-900/30">Rain</Badge>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs">Optimal Planting (Rain + Planting)</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Guidelines for loan officers based on crop and climate data</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <h3 className="text-sm font-medium mb-2">Loan Timing Recommendations</h3>
            <p className="text-sm">
              {isSuitable() 
                ? `Loans for ${selectedCrop} cultivation in the ${zoneInfo?.name} are recommended during ${cropInfo?.monthNames.join(', ')} to align with optimal planting times.`
          ...
          <div>
            <h3 className="text-sm font-medium mb-2">Risk Factors to Consider</h3>
            <ul className="space-y-2">
              {cropInfo?.needsIrrigation && (
                <li className="text-sm flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  <span>Verify that the farmer has access to reliable irrigation systems, as ${selectedCrop} requires consistent water supply.</span>
                </li>
              )}
              {!cropInfo?.plantingMonths.some(month => zoneInfo?.rainyMonths.includes(month)) && (
                <li className="text-sm flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  <span>The planting season for ${selectedCrop} does not align well with the rainy season in this region, which may increase risk.</span>
                </li>
              )}
              <li className="text-sm flex items-start gap-2">
                <span className="text-primary mt-1">•</span>
                <span>Loan repayment schedules should be aligned with expected harvest time (${cropInfo?.harvestMonthNames.join(', ')}) when farmers will have income from crop sales.</span>
              </li>
              <li className="text-sm flex items-start gap-2">
                <span className="text-primary mt-1">•</span>
                <span>Consider the farmer's experience with ${selectedCrop} cultivation and their historical yields.</span>
              </li>
            </ul>
          </div>
          
          <div>
            <h3 className="text-sm font-medium mb-2">Loan Structure Suggestions</h3>
            <ul className="space-y-2">
              <li className="text-sm flex items-start gap-2">
                <span className="text-primary mt-1">•</span>
                <span>Loan disbursement: Consider phased disbursement aligned with critical farming activities (land preparation, planting, fertilization).</span>
              </li>
              <li className="text-sm flex items-start gap-2">
                <span className="text-primary mt-1">•</span>
                <span>Repayment grace period: Allow ${cropInfo?.durationDays / 30} months grace period to align with the crop's maturity timeline.</span>
              </li>
              <li className="text-sm flex items-start gap-2">
                <span className="text-primary mt-1">•</span>
                <span>Loan term: For ${selectedCrop}, a term of ${Math.ceil(cropInfo?.durationDays / 30) + 2} to ${Math.ceil(cropInfo?.durationDays / 30) + 4} months is recommended to cover growing period plus post-harvest sales.</span>
              </li>
            </ul>
          </div>
        </CardContent>
